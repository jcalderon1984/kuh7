    ...
    def download_cfdi_invoices(self, start_date=False, end_Date=False):
        ...
        if invoice_content_emisor:
            ...
            for uuid,data in invoice_content_emisor.items():
                ...
                if cfdi_type=='P':
                    for uu in [uuid,uuid.lower(),uuid.upper()]:
                        payment_exist = payment_obj.search([('folio_fiscal','=',uu),('payment_type','=','inbound')],limit=1)
                        if payment_exist:
                            vals.update({'creado_en_odoo' : True,'payment_ids':[(6,0, payment_exist.ids)]})
                            break
                if cfdi_type=='E':
                    for uu in [uuid,uuid.lower(),uuid.upper()]:
                        invoice_exist = invoice_obj.search([('folio_fiscal','=',uu),('type','=','out_refund')],limit=1)
                        if invoice_exist:
                            vals.update({'creado_en_odoo' : True,'payment_ids':[(6,0, invoice_exist.ids)]})
                            break
                else:
                    for uu in [uuid,uuid.lower(),uuid.upper()]:
                        invoice_exist = invoice_obj.search([('folio_fiscal','=',uu),('type','=','out_invoice')],limit=1)
                        if invoice_exist:
                            vals.update({'creado_en_odoo' : True,'invoice_ids':[(6,0, invoice_exist.ids)]})
                            break
